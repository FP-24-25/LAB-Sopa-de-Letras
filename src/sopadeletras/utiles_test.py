...
def test_inicializar_sopa_vacia():
    ...
    # Verificamos que cada fila es un objeto independiente
    ids_filas = set()
    for fila in sopa_vacia:
        identificador = id(fila)
        assert identificador not in ids_filas, f"Cada fila debe ser una lista independiente de las demás (¿has añadido la misma lista varias veces?)"            
        ids_filas.add(identificador)
